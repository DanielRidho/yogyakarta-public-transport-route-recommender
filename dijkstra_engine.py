import itertools
import math
import networkx as nx
import pandas as pd

from schedule_utils import (
    in_service_window,
    next_departure_wait_min,
    get_traffic_multiplier,
)
from fare_utils import calculate_fare

# Penalti (baseline)
TRANSFER_TAP_PENALTY = 1.0     
ROUTE_CHANGE_PENALTY = 2.0      
INTERMODAL_PENALTY   = 3.0     
DIRECT_ROUTE_BONUS   = -1.0     


# =========================================================
# Kandidat jalur berdasarkan travel_time statis di graf
# =========================================================
def k_shortest_paths_by_time(G, source, target, k=3):
    """
    Cari K rute terpendek berdasar atribut 'travel_time' di graf.
    """

    def path_time(p):
        return sum(G[u][v].get("travel_time", 0.0) for u, v in zip(p[:-1], p[1:]))

    try:
        gen = nx.shortest_simple_paths(G, source, target, weight="travel_time")
        paths = list(itertools.islice(gen, max(6, k)))
    except (nx.NetworkXNoPath, nx.NodeNotFound):
        return []

    return sorted(paths, key=path_time)[:k]


# =========================================================
# Group edge menjadi segmen per (mode, route_id)
# =========================================================
def _group_segments(G, path):
    """
    Gabungkan edge-edge bertetangga yang punya mode & route_id sama
    menjadi satu segmen panjang.
    """
    edges = list(zip(path[:-1], path[1:]))
    segs, i = [], 0
    while i < len(edges):
        u, v = edges[i]
        a = G[u][v]
        mode, rid = a.get("mode"), a.get("route_id")
        block = [(u, v)]
        j = i
        last = v
        while j + 1 < len(edges):
            u2, v2 = edges[j + 1]
            a2 = G[u2][v2]
            if a2.get("mode") == mode and a2.get("route_id") == rid:
                block.append((u2, v2))
                j += 1
                last = v2
            else:
                break
        segs.append(
            dict(
                mode=mode,
                route_id=rid,
                start_node=u,
                end_node=last,
                edges=block,
            )
        )
        i = j + 1
    return segs


# =========================================================
# Hitung waktu perjalanan REAL (jadwal + penalti + macet)
# =========================================================
def compute_time(
    G,
    path,
    routes_df,
    timetables_df,
    stops_df,
    start_minute,
    current_day,
    traffic_rules_df=None,
):
    """
    Hitung waktu total perjalanan + detail per segmen untuk 1 path.

    Termasuk:
    - cek jam operasi rute (in_service_window)
    - waktu tunggu timetable/headway
    - penalti ganti moda & koridor
    - efek kemacetan (traffic multiplier) untuk segmen bus
    """
    segs = _group_segments(G, path)
    current = float(start_minute)
    total = 0.0
    details = []
    prev = None

    free_lookup = {
        r["stop_id"]: bool(r.get("transfer_free_default", False))
        for _, r in stops_df.iterrows()
    }

    for seg in segs:
        mode, rid = seg["mode"], seg.get("route_id")
        board = seg["start_node"]

        # waktu riding dasar (tanpa macet)
        ride_base = sum(
            float(G[u][v].get("travel_time_min_base", 0.0)) for u, v in seg["edges"]
        )

        # akses (tap awal / jalan pendek dari edge)
        access = float(
            G[seg["edges"][0][0]][seg["edges"][0][1]].get(
                "transfer_penalty_min", 0.0
            )
        )

        wait, extra = 0.0, 0.0

        # Cek jam operasi rute
        if rid and not in_service_window(routes_df, rid, int(current)):
            return math.inf, []

        # Estimasi waktu tunggu — jadwal/headway
        if mode in ("bus", "krl", "prameks", "railink"):
            wait = next_departure_wait_min(
                route_id=rid,
                stop_id=board,
                timetables_df=timetables_df,
                current_minute=int(current),
                current_day=current_day,
                routes_df=routes_df,
                mode=mode,
            )
            if wait is None or math.isinf(wait):
                return math.inf, []
        else:
            wait = 0.0

        # Efek macet (traffic multiplier) → diterapkan ke ride time
        ride = ride_base
        if traffic_rules_df is not None and mode == "bus":
            # hanya bus yang kena kemacetan jalan
            m = get_traffic_multiplier(int(current + wait), current_day, traffic_rules_df)
            ride = ride_base * float(m)

        # Penalti pergantian moda/rute
        if prev:
            if prev["mode"] != mode:
                extra += INTERMODAL_PENALTY
                if prev["mode"] == "bus" and not free_lookup.get(board, False):
                    extra += TRANSFER_TAP_PENALTY

            if (
                prev["mode"] == "bus"
                and mode == "bus"
                and seg.get("route_id") != prev.get("route_id")
            ):
                extra += ROUTE_CHANGE_PENALTY
                if not free_lookup.get(board, False):
                    extra += TRANSFER_TAP_PENALTY

            if prev.get("route_id") == rid and rid is not None:
                extra += DIRECT_ROUTE_BONUS

        # Total waktu segmen ini
        seg_time = access + wait + ride + extra
        total += seg_time
        current += seg_time

        details.append(
            dict(
                **seg,
                access_over_min=round(access, 2),
                wait_min=round(wait, 2),
                ride_min=round(ride, 2),  # sudah termasuk efek macet
                transfer_penalty_min=round(extra, 2),
                start_clock_min=current - seg_time,
                end_clock_min=current,
            )
        )
        prev = seg

    return round(total, 2), details


# =========================================================
# Evaluasi waktu + biaya + transfer (tanpa fuzzy)
# =========================================================
def evaluate_paths(
    G,
    paths,
    payment_pref,
    fares_df,
    fare_rules_df,
    stops_df,
    routes_df,
    timetables_df,
    start_minute,
    current_day,
    traffic_rules_df=None,
):
    # Evaluasi kumpulan path (hasil k_shortest_paths_by_time).
    results = []

    for p in paths:
        t, segs = compute_time(
            G,
            p,
            routes_df,
            timetables_df,
            stops_df,
            start_minute,
            current_day,
            traffic_rules_df=traffic_rules_df,
        )
        if math.isinf(t):
            continue

        total_cost = 0.0
        prev = None
        active_routes = []

        for seg in segs:
            mode = seg["mode"]
            rid = seg.get("route_id")

            cost = calculate_fare(
                mode,
                rid,
                fares_df,
                fare_rules_df,
                payment_pref,
                u_stop=seg["start_node"],
                v_stop=seg["end_node"],
                current_day=current_day,
            )

            if prev and prev.get("route_id") == rid and prev.get("mode") == mode:
                cost = 0.0

            total_cost += cost
            seg["segment_cost"] = int(round(cost, 0))

            if mode not in ("walk", "ojol") and rid:
                key = f"{mode}_{rid}"
                if key not in active_routes:
                    active_routes.append(key)

            prev = seg

        transfers = len(active_routes)

        results.append(
            dict(
                path=p,
                segments=segs,
                total_time=t,
                total_cost=int(round(total_cost, 0)),
                transfer_count=transfers,
            )
        )

    return sorted(results, key=lambda x: x["total_time"])
