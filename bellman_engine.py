...
def k_shortest_paths_by_time(G, source, target, k=3):
    ...
    for _ in range(max(6, k)):
        ...
        for _, _, d in G.edges(data=True):
            base = d["travel_time"]
            d["travel_time_backup"] = base
            d["travel_time"] = base + d.get("_temp_added",0)

        p = _bf_one(G, source, target)
        ...
        for u,v in zip(p[:-1],p[1:]):
            used_edge_penalty[(u,v)] = used_edge_penalty.get((u,v),0) + 999

        if len(sols) >= k:
            break

    def path_cost(p):
        return sum(G[u][v]["travel_time"] for u,v in zip(p[:-1],p[1:]))
...
def compute_time(G, path, routes_df, timetables_df, stops_df,
    ...
    for seg in segs:
        ...
        ride_base = sum(
            float(G[u][v].get("travel_time_min_base",0))
